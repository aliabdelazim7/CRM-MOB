...
export default function PurchaseInvoicesPage() {
  ...
  const handleAddItem = () => {
    if (products.length > 0) {
      setItems([...items, { product_id: products[0].id, quantity: 1, unit_cost: products[0].purchase_price || 0, tax_percent: 0, discount_percent: 0 }]);
    }
  };
  ...
  const filteredInvoices = advPurchaseInvoices.filter((i) =>
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6" dir="rtl">
                  ...
                  return (
                    <tr key={inv.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition">
                      ...
                      <td className="p-4">
                        ...
                        >
                          {inv.status === 'approved' ? 'مكتملة ومعتمدة (WACC Updated)' : 'مسودة'}
                        </span>
                      </td>
                      <td className="p-4">
                        {inv.status === 'draft' && (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 max-w-3xl w-full space-y-4 shadow-2xl border border-slate-100 dark:border-slate-700 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-black text-slate-800 dark:text-white">إنشاء فاتورة شراء تكليفية متقدمة</h3>
            ...
            <form onSubmit={handleSaveInvoice} className="space-y-4 text-sm">
              ...
              <div>
                ...
                <div className="space-y-2 max-h-52 overflow-y-auto p-2 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700">
                  {items.map((item, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const next = [...items];
                            next[idx].discount_percent = Number(e.target.value);
                            setItems(next);
                          }}
                          className="w-16 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-xs font-bold text-center"
                        />
